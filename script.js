...
const PURPLE = { r: 0x8a, g: 0x2b, b: 0xe2 };
const YELLOW = { r: 0xff, g: 0xd7, b: 0x00 };
const CYAN =   { r: 0x00, g: 0xff, b: 0xff };
const RED =    { r: 0xff, g: 0x33, b: 0x33 };
const GOLD =   { r: 0xff, g: 0xc0, b: 0x00 };
const SILVER = { r: 0xc0, g: 0xc0, b: 0xc0 };
const MAGENTA = { r: 0xff, g: 0x00, b: 0xff };
const TEAL =   { r: 0x00, g: 0x80, b: 0x80 };
const allColors = [BLUE, GREEN, PINK, ORANGE, PURPLE, YELLOW, CYAN, RED, GOLD, SILVER, MAGENTA, TEAL];
...
const createBurst = (() => {
	...
	function getFragForTarget(target) {
		...
		if (!frag) {
			// Pick a random color from our expanded color palette
			const randomColor = pickOne(allColors);
			frag = new Entity({
				model: makeCubeModel({ scale: fragRadius }),
				color: randomColor,
				wireframe: target.wireframe
			});
			frag.color = randomColor;
